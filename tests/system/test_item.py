...
class ItemTest(BaseTest):
    def setUp(self):
        ...
        with self.app() as client:
            with self.app_context():
                ...
                auth_request = client.post("/auth", data=json.dumps({"username": "test", "password": "abcd"}),
    ...
    def test_create_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel("test").save_to_db()
                response=client.post("/item/test", data={"price": 20, "store_id": 1} )
                self.assertEqual(response.status_code, 201)
                self.assertDictEqual({"name": "test", "price": 20}, json.loads(response.data))
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel("test").save_to_db()
                response=client.put("/item/test", data={"price": 20, "store_id": 1})
                self.assertEqual(response.status_code, 200)
    ...
    def test_put_update_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertEqual(ItemModel.find_by_name("test").price, 10)
                response = client.put("/item/test", data={"price": 20, "store_id": 1})
                self.assertEqual(response.status_code, 200)
                self.assertEqual(ItemModel.find_by_name("test").price, 20)
                self.assertDictEqual({"name": "test", "price": 20}, json.loads(response.data))
    def test_item_list(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({"items": [{"name": "test", "price": 10}]}, json.loads(response.data))